...
class CacheConfig:
    ...
    def _get_default_config(self) -> dict[str, Any]:
        ...
        return {
            ...
            "cache_tiers": {
                ...
                "l2_redis": {"enabled": True, "ttl_hours": 2, "serialization_format": "json"},
                "session": {
                    "enabled": True,
                    "ttl_hours": 24,
    ...
    def _validate_config(self) -> None:
        ...
        try:
            ...
            assert isinstance(
                redis_config.get("max_connections"), int
            ), "Max connections must be integer"
            ...
            for tier_name, tier_config in cache_tiers.items():
                assert isinstance(
                    tier_config.get("ttl_hours"), (int, float)
                ), f"{tier_name} TTL must be numeric"
                ...
                try:
                    ...
                    logger.warning(
                        f"Invalid serialization format for {tier_name}: {format_str}, using json"
                    )
    ...
    def get_tier_config(self, tier: CacheTier | str) -> dict[str, Any]:
    ...
    def get_serialization_format(self, tier: CacheTier | str) -> SerializationFormat:
    ...
    def is_tier_enabled(self, tier: CacheTier | str) -> bool:
        """Check if cache tier is enabled."""
        tier_config = self.get_tier_config(tier)
        return tier_config.get("enabled", False)

    def get_ttl_hours(self, tier: CacheTier | str) -> float:
    ...
    def get_performance_recommendations(self) -> dict[str, Any]:
        ...
        return {
            ...
            "config_summary": {
                ...
                "compression_enabled": config.get("optimization", {}).get(
...
_cache_config_instance: CacheConfig | None = None
