...
class SessionManager:
    ...
    def __init__(
        ...
        # Use Redis Hash format for sessions — force db=2 for session isolation
    ...
    def get_session(self, session_id: str) -> dict[str, Any] | None:
        ...
        try:
            session_key = self._get_session_key(session_id)
            session_data = self._redis.get(key=session_key, namespace=self._session_namespace)
    ...
    def update_session(
        ...
        try:
            ...
            updated_data["last_accessed"] = datetime.now(timezone.utc).isoformat()
    ...
    def increment_query_count(self, session_id: str) -> bool:
        """Increment query count for session using atomic Redis HINCRBY."""
        try:
            client = self._redis.get_raw_client()
            if client is None:
                return False
            session_key = self._get_session_key(session_id)
            full_key = self._redis._get_key(session_key, self._session_namespace)
            client.hincrby(full_key, "query_count", 1)
            client.hincrby(full_key, "access_count", 1)
            client.hset(full_key, "last_accessed", datetime.now(timezone.utc).isoformat())
            return True
    ...
    def extend_session(self, session_id: str, hours: int | None = None) -> bool:
        """Extend session TTL."""
        try:
            session_key = self._get_session_key(session_id)
    ...
    def _track_user_session(self, user_id: str, session_id: str) -> None:
        ...
        try:
            ...
            self._redis.set_add(user_sessions_key, session_id)
            self._redis.expire(user_sessions_key, self.session_ttl_hours)
        except Exception as e:
    ...
    def get_user_sessions(self, user_id: str) -> list[str]:
        ...
        try:
            user_sessions_key = f"user_sessions:{user_id}"
            sessions = self._redis.set_members(user_sessions_key)
        ...
        try:
            ...
            for session_key in session_keys[:100]:  # Sample first 100 for performance
                session_data = self.get_session(
                    session_key.replace(f"{self._session_namespace}:", "")
                )
                if session_data:
                    ...
                    try:
                        last_access = datetime.fromisoformat(
                            str(session_data.get("last_accessed", "1970-01-01T00:00:00")).replace(
                                "Z", "+00:00"
                            )
                        )
    ...
    def clear_all_sessions(self) -> bool:
        ...
        try:
            session_keys = self._redis.keys("*", namespace=self._session_namespace)
            if session_keys:
                result = self._redis.delete_multi(session_keys, namespace=self._session_namespace)
