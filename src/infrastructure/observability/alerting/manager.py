...
class AlertManager:
    ...
    def __init__(self, metrics_collector=None):
        ...
        self._runbook_base = os.getenv("RUNBOOK_BASE_URL", "")
    ...
    def _init_default_rules(self):
        ...
        default_rules = [
            AlertRule(
                ...
                annotations={"runbook_url": f"{self._runbook_base}/runbooks/high-error-rate"},
            ...
            AlertRule(
                ...
                labels={"component": "api"},
                annotations={"runbook_url": f"{self._runbook_base}/runbooks/high-response-time"},
                evaluate_interval_seconds=60,
                for_duration_seconds=300,
            ),
            AlertRule(
                ...
                annotations={"runbook_url": f"{self._runbook_base}/runbooks/low-cache-hit-rate"},
            ...
            AlertRule(
                ...
                annotations={"runbook_url": f"{self._runbook_base}/runbooks/high-memory-usage"},
                evaluate_interval_seconds=300,
                for_duration_seconds=300,
            ),
            AlertRule(
                ...
                labels={"component": "system"},
                annotations={"runbook_url": f"{self._runbook_base}/runbooks/system-down"},
                evaluate_interval_seconds=60,
    ...
    def remove_notification_channel(self, channel_name: str):
        ...
        self.notification_channels = [
            c for c in self.notification_channels if c.name != channel_name
        ]
    ...
    def silence_alert(self, alert_name: str, duration_minutes: int = 60):
        ...
        silence_until = datetime.now(timezone.utc) + timedelta(minutes=duration_minutes)
    ...
    def _get_metrics_for_rule(self, _rule: AlertRule) -> dict[str, Any]:
    ...
    def _check_response_time(self, _metrics_data: dict[str, Any]) -> bool:
        """Check if response time is high."""
        try:
            # Get 95th percentile response time
            stats = self.metrics_collector.get_histogram_stats(
                "request_duration_seconds", time_window_minutes=5
            )
            p95 = stats.get("p95", 0)
...
def get_alert_manager(metrics_collector=None) -> AlertManager | None:
